...
function EditServicePage(props: {}) {
    ...
    return <Container id={'editContainer'} style={{textAlign: "left"}}>
        ...
        <Tabs id={'outerTabBar'} activeKey={key} onSelect={(e) => e && setKey(e)} fill>
            {
                userApp?.services?.map((svc, index) => <Tab title={svc.service} eventKey={svc.service}>
                    <Tab.Container id={'innerTabBar'} activeKey={innerKey} onSelect={(e) => e && setInnerKey(e)}>
                        <Row className={'marginTop'}>
                            <Col sm={3}>
                                <Nav variant="pills" className="flex-column">
                                    <Nav.Item>
                                        {tabNames.map(name => <Nav.Link eventKey={name}>{name}</Nav.Link>)}
                                    </Nav.Item>
                                </Nav>
                            </Col>
                            <Col sm={9}>
                                <Tab.Content>
                                    <Tab.Pane eventKey="Environment Variables">
                                        <Form>
                                            ...
                                            {
                                                vars.filter(s => svc.service === s.service).map(sv => <><Table striped bordered size={'sm'}>
                                                    {
                                                        sv?.env?.map((e, envIndex) => <tr>
                                                            <td>
                                                                <Form.Group controlId="envName">
                                                                    <Form.Control required id={'envName'} name={'envName'} type="text" placeholder="Name is required" value={e.name} onChange={(e) => handleChange(e, svc.service, envIndex, 'name')} />
                                                                </Form.Group>
                                                            </td>
                                                            <td>
                                                                <Form.Group controlId="envVal">
                                                                    <Form.Control type="text" id={'envValue'} name={'envValue'} placeholder="Enter a value (optional)" value={e.value} onChange={(e) => handleChange(e, svc.service, envIndex, 'value')} />
                                                                </Form.Group>
                                                            </td>
                                                        </tr>)
                                                    }
                                        ...
                                        <Button disabled={disabled()} className={'btn-success'} onClick={save}><FontAwesomeIcon icon={faSave} /></Button>
                                    </Tab.Pane>
